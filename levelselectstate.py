...
# All needed so the level select menu and choosing levels action will work.
    ...
    def create_buttons(self):
        ...
        back_button = Button(self.screen, self.textures["button"].
                             get_rect(center=self.screen.get_rect().
                                      center),
                             [self.textures["button"]],
                             caption="Main Menu", command=self.menu)
        back_button.rect.y += 200
        buttons.append(back_button)
        return buttons

    # Choosing level function
