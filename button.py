...
# All needed for a button (Drawing the button out, fill the text on and allow clicking)
class Button:
    # Initialize the button when the game is running
    def __init__(self, screen, rect, texture=None, caption="",
        ...
        if texture is not None and len(texture) == 1:
            texture = 3 * texture
        self.button_surface = pg.surface.Surface((rect.width, rect.height),pg.SRCALPHA, 32)
        self.screen = screen
        ...
        self.text_rect = self.text_surface.get_rect(center=(rect.width/2,rect.height/2))
        self.command = command
        self.screen = screen
        self.args = args

    # Draw the button down (It's ugly sorry for that :))
    def draw(self):
