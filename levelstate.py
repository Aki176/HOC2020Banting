...
# Most important module to manage the game when you are playing
# LevelState for drawing UI, game logic and managing turrets, enemies, projectiles
# Turret for drawing, managing turrets
# Enemy for drawing, managing enemies
# Projectile for drawing, managing projectiles
# EnemySpawner for creating new enemies
# You can find value like base health, base money and many more like upgrade money, upgrade stats 
#in those modules I listed up there down here.


# Important class to manage what happen when you are playing the game. 
class LevelState(State):
    ...
    def startup(self, screen, level_name, reset=True):
        ...
        (self.map_rect, self.panel_rect, self.stat_rect, self.turrets_rect,self.info_rect, self.button_rect) = self.prepare_areas()
        (self.map_surface, self.tiles,self.paths, self.spawn_point) = self.prepare_map()
        self.panel_surface = Surface(self.panel_rect.size)
        self.panel_surface.blit(self.textures["panel_background"],(0, 0))
        ...
        # Initialize turret
        Turret.init(self.screen, self.turrets, self.enemies, self.projectiles,self.resource_manager)
        ...
        # Get data from level_data.json file
        self.max_wave = self.level_data["waves"]
        self.health = self.level_data["health"]
        self.money = self.level_data["start_money"]

        self.next_wave_time = 0 # Next wave time
        self.current_wave = 0 
        self.speed_multiplier = 1 # Speed default

        self.enemy_spawner = EnemySpawner(screen, self.enemies,
                                          self.spawn_point) # Spawn enemy

        self.create_buttons() # Create button in game screen
    ...
    def update(self, dt):
        ...
        # What happen when you win a level (beat all waves)
        if (self.finished is False and self.alive_enemies == 0 and
            ...
            self.buttons.append(Button(self.screen, self.screen.get_rect(),
                                caption="You have won the level, click anywhere to return to main menu",
                                command=self.go_to_menu))
            self.finished = True

        # What happen when you lost a level (lost all health)
        if (self.finished is False and self.health <= 0):
            for button in self.buttons:
                button.toggle_active()
            self.buttons.append(Button(self.screen, self.screen.get_rect(),
                                caption="You have lost the level, click anywhere to return to main menu",
                                command=self.go_to_menu))
            self.finished = True

    # After finish a level then click anywhere to go to main meny
    def go_to_menu(self):
        self.done = True
        self.next = "menu"
    ...
    # Write down stats like Money, HP and current Wave
    def draw_stats(self):
        white = (255, 255, 255)
        ...
        money_text1 = self.BASICFONT.render("Vallis", True, white)
        money_text2 = self.BASICFONT.render(str(self.money), True, white)

        health_text1 = self.BASICFONT.render("Health", True, white)
        health_text2 = self.BASICFONT.render(str(self.health), True, white)

        wave_text1 = self.BASICFONT.render("Wave", True, white)
        ...
        self.screen.blit(money_text1, money_text1.get_rect(
                         midleft=money_rect.midleft)) # Money pos
        ...
        self.screen.blit(health_text1, health_text1.get_rect(
                         midleft=health_rect.midleft)) # HP pos
        ...
        self.screen.blit(wave_text1, wave_text1.get_rect(
                         midleft=wave_rect.midleft)) # Wave pos
    ...
    def create_buttons(self):
        ...
        self.buttons.append(Button(self.screen,
                            self.textures["turret0"].get_rect(
                                topleft=(x + 43 * 2+64, 28 + y)), # Turret pos
        ...
        self.buttons.append(Button(self.screen,
                            ...
                            command=lambda: self.set_speed_multiplier(2))) # X2 speed

        self.buttons.append(Button(self.screen,
                            self.textures["speed1"].get_rect(
                                midright=self.buttons[-1].rect.midleft),
                            texture=[self.textures["speed1"]],
                            command=lambda: self.set_speed_multiplier(1))) # Default speed

        self.buttons.append(Button(self.screen,
                            self.textures["speed0"].get_rect(
                                midright=self.buttons[-1].rect.midleft),
                            texture=[self.textures["speed0"]],
                            command=lambda: self.set_speed_multiplier(0))) # Pause the game
    ...
    def describe_turret(self):
        ...
        dmg_text1 = self.BASICFONT.render("Damage", True, white)
        dmg_text2 = self.BASICFONT.render(str(selected_turret.dmg), True,
                                          white)

        range_text1 = self.BASICFONT.render("Range", True, white)
        range_text2 = self.BASICFONT.render(str(selected_turret.range), True,
                                            white)

        wave_text1 = self.BASICFONT.render("Attack Speed", True, white)
    ...
    # Sell turret function, default you will get 40% amount of money
...
class Turret:
    ...
    #             DMG RANGE ATKSPD
    BASE_STATS = [(5, 400, 0.1), # Base stats turret 1
                  (3, 200, 0.02), # Base stats turret 2
                  (10, 700, 1)] # Base stats turret 3
...
class Enemy:
    ...
    #             HP SPEED VALUE
    BASE_STATS = [(10, 700, 3), # Enemy 1 Stats
                  (5, 1000, 3), # Enemy 2 Stats
                  (60, 500, 3)] # Enemy 3 stats
